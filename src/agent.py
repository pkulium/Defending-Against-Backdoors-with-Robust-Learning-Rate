...
class Agent():
    ...
    def train_mask(self, global_model, criterion):
        ...
        save_mask_scores(self.local_model.state_dict(), f'/work/LAS/wzhang-lab/mingl/code/backdoor/Defending-Against-Backdoors-with-Robust-Learning-Rate/save/mask_values{self.id}.txt')
        mask_values = read_data(f'/work/LAS/wzhang-lab/mingl/code/backdoor/Defending-Against-Backdoors-with-Robust-Learning-Rate/save/mask_values{self.id}.txt')
        mask_values = sorted(mask_values, key=lambda x: float(x[2]))
        print(f'mask_values:{mask_values[0]} - {mask_values[100]} - {mask_values[1000]}')
